...
void *wrapper_amplitude(void *thread_result) {
  struct bl_song const *const song =
      ((struct thread_result_s *)thread_result)->song;
...
void *wrapper_tempo(void *thread_result) {
  struct bl_song const *const song =
      ((struct thread_envelope_result_s *)thread_result)->song;
  bl_envelope_sort(
      song, (((struct thread_envelope_result_s *)thread_result)->results));

  return 0;
}

int bl_analyze(char const *const filename, struct bl_song *const current_song) {
  ...
  if (bl_audio_decode(filename, current_song) == BL_OK) {
    ...
    pthread_create(&analyze_thread, NULL, wrapper_amplitude,
                   (void *)&amplitude_result);
    ...
    pthread_create(&frequency_thread, NULL, wrapper_frequency,
                   (void *)&frequency_result);

    // Analyze global envelope
    struct thread_envelope_result_s envelope_results = {current_song,
                                                        &envelope_result};
    pthread_create(&envelope_thread, NULL, wrapper_tempo,
                   (void *)&envelope_results);
...
float bl_distance(struct force_vector_s v_song1,
  ...
  distance =
      sqrt((v1.tempo - v2.tempo) * (v1.tempo - v2.tempo) +
...
float bl_distance_file(char const *const filename1, char const *const filename2,
                       struct bl_song *song1, struct bl_song *song2) {
  struct force_vector_s v1, v2;
  float distance;
